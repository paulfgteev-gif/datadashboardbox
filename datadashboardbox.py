...
class window(QDialog):
    ...
    def init_serial(self):
        try:
            # We don't use time.sleep here because it freezes the UI.
            # Instead, we just open the port.
            self.arduino = serial.Serial('/dev/ttyACM0', 9600, timeout=0.1)
            print("Port opened. Arduino may reset...")
        except Exception as e:
            print(f"Connection Error: {e}")

        # 3. Connect Buttons (Check ObjectNames in Qt Designer)
        # Using lambda to pass the character to the function
